    ...
    >
      <div className="glass-card rounded-2xl p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-sm">Sign in to continue preparing</p>
        </div>
        ...
        <form onSubmit={handleCredentials} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <Input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" className="pl-10 bg-white/5 border-white/20" required />
            </div>
          </div>
          <div>
            <Label htmlFor="password" className="text-gray-300 text-sm mb-1.5 block">Password</Label>
            <div className="relative">
              <Input id="password" type={showPwd ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" className="pr-10 bg-white/5 border-white/20" required />
          ...
          <Button type="submit" variant="gradient" className="w-full" disabled={!!loading}>
            {loading === 'credentials' ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : null}
            Sign In
          </Button>
        </form>
      </div>
